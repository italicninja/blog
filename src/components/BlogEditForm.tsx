...
export default function BlogEditForm({ post }: BlogEditFormProps) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (coverImage) {
        try {
          // Only convert if it's a new URL (not already metadata)
          if (!coverImage.startsWith('{')) {
            coverImageMetadata = imageUrlToMetadata(coverImage, "Cover image for post");
          } else {
            // Already metadata, use as is
            coverImageMetadata = coverImage;
          }
        } catch (error) {
      ...
      // Redirect to the updated post after a delay
      setTimeout(() => {
    ...
    return (
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6 text-center">
        ...
        <p className="text-yellow-700">
          You need to be signed in with GitHub to edit a blog post.
        </p>
        <button
    ...
    <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm p-4">
      ...
      {success && (
        <div className="mb-4 p-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-md text-green-600 dark:text-green-400">
          Blog post updated successfully! Redirecting to your post...
        </div>
      )}

      {processingImages && imageProcessingStatus && (
        <div className="mb-4 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-md text-blue-600 dark:text-blue-400">
      ...
      <form ref={formRef} onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex gap-4">
          ...
          <button
            ...
            className="flex-1 px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 transition-colors disabled:opacity-70 disabled:cursor-not-allowed"
          >
            {isSubmitting ? (
