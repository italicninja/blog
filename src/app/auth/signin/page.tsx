  ...
  useEffect(() => {
    const autoSignIn = async () => {
      if (isDevelopment) {
        setIsLoading(true);
        try {
          const result = await signIn('dev-bypass', { callbackUrl, redirect: false });
          if (result?.error) {
          ...
          <div className="max-w-md mx-auto bg-background border border-gray-200 dark:border-gray-800 rounded-lg shadow-sm p-8">
            ...
            {isDevelopment ? (
              <div className="text-center text-sm text-gray-600 dark:text-gray-400">
                {isLoading ? (
                  <>
                    <svg className="animate-spin h-5 w-5 text-gray-600 dark:text-gray-400 mx-auto mb-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                      <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                      <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                    </svg>
                    Automatically signing in with development account...
                  </>
                ) : (
                  "Development mode: Auto sign-in complete. You should be redirected shortly."
                )}
              </div>
            ) : (
              <button
